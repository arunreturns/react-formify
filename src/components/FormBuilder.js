...
class FormBuilder extends React.Component {
  ...
  validateForm() {
      ...
      switch (validator.type) {
        case "ALL":
          for (let k = 0; k < validator.requiredFlds.length; k++) {
            let err = this.validateField(fieldMap[validator.requiredFlds[k]], this.props.formObject);
            if (err) {
              errors.push({
        ...
        case "CO-MANDATORY":
          for (let l = 0; l < validator.parentFields.length; l++) {
            ...
            let formValue = this.props.formObject[fldName] || "";
            if (formValue && (formValue.indexOf(fldValue) >= 0 || formValue === fldValue)) {
              ...
              for (let m = 0; m < validator.childFields[l].length; m++) {
                ...
                let { fldName, fldValue } = childField;
                formValue = this.props.formObject[fldName] || "";
                if (Array.isArray(fldValue)) {
                  // Either or combination. Any one of the value should be present
                  if (!formValue || fldValue.indexOf(formValue) === -1) {
                    errors.push({
                      errorType: `COMANDATORY_REQUIRED_${l}_${m}`,
                      errorMsg: `Missing required value for ${fldName}`
                    });
                    continue;
                  }
                } else {
                  ...
                  if (Array.isArray(formValue)) {
                    if (!formValue || formValue.indexOf(fldValue) === -1) {
                      errors.push({
                        errorType: `COMANDATORY_REQUIRED_${l}_${m}`,
                        errorMsg: `Missing required value ${fldValue} for ${fldName}`
                      });
                      continue;
                    }
                  } else {
                    if (!formValue || fldValue !== formValue) {
                      errors.push({
                        errorType: `COMANDATORY_REQUIRED_${l}_${m}`,
                        errorMsg: `Missing required value ${fldValue} for ${fldName}`
                      });
    ...
    if ( errors.length > 0 ){
      ...
      this.props.handleFormSubmit(this.state.formObject)
    }
  }
  ...
  checkOptionAvailability(val, field) {
    ...
    fldOptions = this.filterHiddenOptions(hiddenOptions, fldOptions);
    if (fldOptions.indexOf(val) >= 0) return val;
    // Remove the disabled values
    fldOptions = this.filterDisabledOptions(disabledOptions, fldOptions);
    if (fldOptions.indexOf(val) >= 0) return val;
    return;
  }
  ...
  filterHiddenOptions(hiddenOptions, fldOptions) {
    let options = fldOptions;
    if (typeof fldOptions === 'undefined') return [];
    for (let hiddenKey in hiddenOptions) {
      let hiddenParent = hiddenOptions[hiddenKey];
      for (let hiddenValue in hiddenParent) {
        let formValue = this.props.formObject[hiddenKey];
        formValue = Array.isArray(formValue) ? formValue : [formValue];
        formValue.forEach(val => {
          if (val === hiddenValue) {
            options = options.filter(opt => hiddenParent[hiddenValue].indexOf(opt) === -1)
          }
        })
      }
    }
  ...
  filterDisabledOptions(disabledOptions, fldOptions) {
    let options = fldOptions;
    if (typeof fldOptions === 'undefined') return [];
    for (let disabledKey in disabledOptions) {
      let disabledParent = disabledOptions[disabledKey];
      for (let disabledValue in disabledParent) {
        let formValue = this.props.formObject[disabledKey];
        formValue = Array.isArray(formValue) ? formValue : [formValue];
        formValue.forEach(val => {
          if (val === disabledValue) {
            options = this.disableOption(options, disabledParent, disabledValue);
          }
        })
      }
    }
